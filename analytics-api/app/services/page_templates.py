...
class PageTemplate:
    """Immutable page template definition."""
    id: str
    name: str
    icon: str
    description: str
...
_OVERVIEW = PageTemplate(
    id="overview",
    name="Overview",
    icon="chart-bar",
    description="High-level KPIs, usage trends, and team breakdown",
    layout=[
        {
            "id": "overview-kpis",
            "columns": 4,
...
_USAGE = PageTemplate(
    ...
    name="Usage & Adoption",
    icon="users",
    description="User adoption rates, agent distribution, and top users",
    layout=[
        {
            "id": "usage-top",
            "columns": 2,
...
_COST = PageTemplate(
    ...
    description="Budget tracking, cost trends, and token usage",
    layout=[
        {
            "id": "cost-budget",
            "columns": 1,
...
_PERFORMANCE = PageTemplate(
    id="performance-reliability",
    name="Performance & Reliability",
    icon="bolt",
    description="Success rates, latency percentiles, and error analysis",
    layout=[
        {
            "id": "perf-kpi",
            "columns": 1,
...
# Ordered list — used for seeding default pages and listing templates.
TEMPLATE_LIST: list[PageTemplate] = [_OVERVIEW, _USAGE, _COST, _PERFORMANCE]

# Indexed map — O(1) lookup by template ID.
TEMPLATES: dict[str, PageTemplate] = {t.id: t for t in TEMPLATE_LIST}
